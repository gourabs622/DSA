...
inline void solve()
{
    ...
    {
        while (!q1.empty() && q1.front().F <= v[i])
        {
            q1.pop_back();
        }
        q1.push_back({v[i], i});
        while (!q2.empty() && q2.front().F >= v[i])
        {
            q2.pop_back();
        }
        q2.push_back({v[i], i});
    }
    // cout << q1.front().F << " " << q2.front().F << dl;
    sum += (q1.front().F + q2.front().F);
    fi(i, k, n)
    {
        while (!q1.empty() && q1.front().F <= v[i])
        {
