...
inline int LRS(string s, int i, int j) // Recursion (TC --> O(exponential) and SC --> O(no of recursion))
{
...
inline int LRSDP(string s) // DP --> TopDown (TC --> O(n^2) and SC --> O(n^2))
{
    mm(gs, 0);
    fi(i, 1, s.length() + 1)
    {
