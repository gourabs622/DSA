...
// Problem Link --> https://www.geeksforgeeks.org/count-operations-make-stringab-free/
...
{
    fast();
    string s; // aabbaaaaabbbbaaaaabbbb
    cin >> s;

    /* Naive Approach --> O(exponential) and constant space */
    // int res = 0, i = 0;
    // while (i < s.length() - 1)
    // {
    //     if (s[i] == 'a' && s[i + 1] == 'b')
    ...
    //         string tmp2 = s.substr(i + 2, s.length() - i);
    //         s = tmp1 + "bba" + tmp2;
    //         res++;
    //         i = -1;
    //     }
    ...
    /* Efficient Approach --> https://www.geeksforgeeks.org/count-operations-make-stringab-free/ */
}
