...
// inline int seqPatMatch(string a, string b, int i, int j)//wrong
// {
//     if (i == 0 || j == 0)
//     {
//         return 0;
...
//     if (a[i - 1] == b[j - 1])
//     {
//         return 1 + seqPatMatch(a, b, i - 1, j - 1);
//     }
//     else
//     {
//         return max(seqPatMatch(a, b, i - 1, j), seqPatMatch(a, b, i, j - 1));
//     }
// }
...
inline void solve()
{
    ...
    int n = a.length();
    int m = b.length();
    // (seqPatMatch(a, b, a.length(), b.length()) != 0) ? cout << "True" << dl : cout << "False" << dl;
    //LCS approach
    fi(i, 0, n + 1)
    {
        gs[i][0] = 0;
    ...
    int k = min(n, m);
    (gs[n][m] == k) ? cout << "True" << dl : cout << "False" << dl;
}
