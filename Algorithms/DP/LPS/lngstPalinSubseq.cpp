#pragma GCC optimize("Ofast")
#pragma GCC target("avx,avx2,fma")
#include <bits/stdc++.h>

/*          					GOURAB SARKAR					            */

using namespace std;

typedef long long ll;
typedef long double ld;
#define PI 3.141592653589793238
#define MP make_pair
#define F first
#define S second
#define PB push_back
#define PBO pop_back()
#define permute next_permutation
#define ALL(v) v.begin(), v.end()
#define SORT(v) sort(ALL(v))
#define REVERSE(v) reverse(ALL(v))
#define ALLA(arr, sz) arr, arr + sz
#define SORTA(arr, sz) sort(ALLA(arr, sz))
#define REVERSEA(arr, sz) reverse(ALLA(arr, sz))
#define dl "\n"
#define mm(a, val) memset(a, val, sizeof(a))
#define b_s binary_search
#define u_b upper_bound
#define l_b lower_bound
#define test  \
    int t;    \
    cin >> t; \
    while (t--)
#define fast()                        \
    ios_base::sync_with_stdio(false); \
    cin.tie(NULL);                    \
    cout.tie(NULL);

const ld EPS = 1e-9;
const int N = 1000000005;
const ll NL = 1000000000000000005LL;
const int mod = 1e9 + 7;
const int mod1 = 1e9 + 9;
const int mod2 = 998244353;

#define fi(a, b, c) for (int a = b; a < c; ++a)
#define fd(a, b, c) for (int a = c - 1; a >= b; --a)
#define fei(a, b, c) for (int a = b; a <= c; ++a) //for any particular range
#define fed(a, b, c) for (int a = c; a >= b; --a) //for any particular range
#define fs(a, b) for (auto &a : b)                //for loop shortcut

// inline void OPEN()
// {
//     #ifdef ONLINE_JUDGE
//         freopen("input.in", "r", stdin);
//         freopen("output.in", "w", stdout);
//     #endif
// }

// const int maxN = 1e5 + 1;
// vector<int> adj[N];
// vector<bool> vis(N, false);

int gs[10001][10001];
int gs[2][10001];//for space optimized

inline int lps(string a, int i, int j)
{
    if (i > j)
    {
        return 0;
    }
    if (i == j)
    {
        return 1;
    }
    if (a[i] == a[j])
    {
        return 2 + lps(a, i + 1, j - 1);
    }
    else
    {
        return max(lps(a, i + 1, j), lps(a, i, j - 1));
    }
}

inline int lps_TopDown(string a, int i, int j)
{
    if (i > j)
    {
        return 0;
    }
    if (i == j)
    {
        return 1;
    }
    if (a[i] == a[j])
    {
        return gs[i][j] = 2 + lps(a, i + 1, j - 1);
    }
    else
    {
        return gs[i][j] = max(lps(a, i + 1, j), lps(a, i, j - 1));
    }
}

inline void another_indexing(string a)
{
    fi(i, 0, a.length())
    {
        fi(j, 0, a.length())
        {
            // if (i == 0 && j == 0)
            // {
            //     gs[i][j] = 0;
            // }
            if (i == j)
            {
                gs[i][j] = 1;
            }
            else if (i > j)
            {
                gs[i][j] = 0;
            }
        }
    }
    fd(i, 0, a.length() - 2)
    {
        fi(j, i + 1, a.length())
        {
            if (a[i] == a[j])
            {
                gs[i][j] = 2 + gs[i + 1][j - 1];
            }
            else
            {
                gs[i][j] = max(gs[i + 1][j], gs[i][j - 1]);
            }
        }
    }
    fi(i, 0, a.length())
    {
        fi(j, 0, a.length())
        {
            cout << gs[i][j] << " ";
        }
        cout << dl;
    }
    cout << gs[0][a.length()] << dl;
}

inline int LPS_BottomUp_Optimization1(string a) { // space optimized O(2*n)
    if (a.length()==1) {
        return 1;
    }
    gs[1][a.length()]=1;
    fd(i, 1, a.length()) {
        gs[0][i]=1;
        fi(j, i+1, a.length()+1) {
            if (a[i-1]==a[j-1]) {
                gs[0][j]=2+gs[1][j-1];
            }
            else {
                gs[0][j]=max(gs[0][j-1], gs[1][j]);
            }
        }
        fi(j, 1, a.length()+1) {
            gs[1][j]=gs[0][j];
        }
    }
    return gs[0][a.length()];
}

inline void solve()
{
    fast();
    string a, temp;
    cin >> a;
    // temp = a;
    // REVERSE(a);//for LCS
    // cout << lps(a, 0, a.length() - 1) << dl;
    // mm(gs, -1);
    // cout << lps_TopDown(a, 0, a.length() - 1) << dl;
    fi(i, 0, a.length()+1)
    {
        fi(j, 0, a.length()+1)
        {
            if (i == 0 && j == 0)
            {
                gs[i][j] = 0;
            }
            else if (i == j)
            {
                gs[i][j] = 1;
            }
            else if (i > j)
            {
                gs[i][j] = 0;
            }
        }
    }
    fd(i, 0, a.length() - 1)
    {
        fi(j, i + 1, a.length())
        {
            if (a[i] == a[j])
            {
                gs[i + 1][j + 1] = 2 + gs[i + 2][j];
            }
            else
            {
                gs[i + 1][j + 1] = max(gs[i + 1][j], gs[i + 2][j + 1]);
            }
        }
    }
    fi(i, 0, a.length() + 1)
    {
        fi(j, 0, a.length() + 1)
        {
            cout << gs[i][j] << " ";
        }
        cout << dl;
    }
    cout << gs[1][a.length()] << dl;

    //method 2(using lcs logic)
    // fi(i, 0, a.length() + 1)
    // {
    //     gs[i][0] = 0;
    // }
    // fi(i, 0, a.length() + 1)
    // {
    //     gs[0][i] = 0;
    // }
    // fi(i, 1, a.length() + 1)
    // {
    //     fi(j, 1, a.length() + 1)
    //     {
    //         if (temp[i - 1] == a[j - 1])
    //         {
    //             gs[i][j] = 1 + gs[i - 1][j - 1];
    //         }
    //         else
    //         {
    //             gs[i][j] = max(gs[i - 1][j], gs[i][j - 1]);
    //         }
    //     }
    // }
    // cout << gs[a.length()][a.length()] << dl;
}

int main()
{
    fast();
    // OPEN();
    solve();
    // test
    // {
    //     solve();
    // }

    return 0;
}