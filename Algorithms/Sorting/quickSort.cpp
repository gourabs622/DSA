...
inline int partition(vector<int> &v, int low, int high)
{
    int pivot = v[low], loww = low, highh = high;
    while (loww < highh)
    {
        while (v[loww] <= pivot)
        {
    ...
    swap(v[highh], v[low]); //take care of this swapping
    return highh;
}

inline void quickSort(vector<int> &v, int low, int high)
{
    if (low < high)
    {
        int prtn = partition(v, low, high);
        quickSort(v, low, prtn - 1);
        quickSort(v, prtn + 1, high);
    }
}
