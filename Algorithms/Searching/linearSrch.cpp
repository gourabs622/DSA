...
inline void solve()
{
    ...
    /* Simple Linear Search --> O(n) and constant space */
    // fi(i, 0, n)
    // {
    //     if (v[i] == ele)
    //     {
    ...
    /* Similar to previous but a little bit efficient --> max-O(n) and min-O(1) and constant space */
    /*whenever we found an element bring it to front so next time if we are searching for the same element
    we get it with only one comparison in constant time you say*/
    fi(i, 0, n)
    {
        if (v[i] == ele)
        {
