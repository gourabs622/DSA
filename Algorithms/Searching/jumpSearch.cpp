...
inline void solve()
{
    ...
    /* Jump Search Algorithm --> O(logn) <= Time Complexity <= O(n) and constant space */
    int prev = 0, step = sqrt(n), currP = sqrt(n);
    while (v[min(step, n) - 1] < ele)
    {
        prev = step;
        step += currP;
        if (prev >= n)
        {
            cout << "Element is not present" << dl;
            return;
        }
    }
    while (v[prev] < ele)
    ...
    if (v[prev] == ele)
    {
        cout << "Element is found at " << prev << dl;
        return;
    }
    cout << "Element is not present" << dl;
}
